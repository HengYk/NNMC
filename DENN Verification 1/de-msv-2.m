frame(in_file_1,in_file_2,in_file_3,v,w,r,o,x,b,y,yc,Ek,randInd,main_percent_rand,main_percent_old) and (
float main_percent_old<==0.914600 and float main_percent_rand<==main_percent_old and skip;
struct _iobuf {
char *_ptr and 
int _cnt and 
char *_base and 
...
individual randInd and skip;
 ...
 {
     frame(score_func_h,score_func_alpha_h,score_func_i,score_func_j,score_func_beta_j,score_func_h_2,return) and ( 
     int return<==0 and skip;
     ...
     while( (score_func_j<1) )
     {
         ...
         int score_func_h_2<==0 and skip;
         
         while( (score_func_h_2<5) )
         {
             score_func_beta_j:=score_func_beta_j+w[score_func_h_2,score_func_j]*b[score_func_h_2];
             score_func_h_2:=score_func_h_2+1
 ...
 {
     frame(assign_v_r_w_o_index,assign_v_r_w_o_i,assign_v_r_w_o_j_1,assign_v_r_w_o_j_2) and ( 
     ...
     while( (assign_v_r_w_o_i<3) )
     {
         ...
         while( (assign_v_r_w_o_j_1<5) )
         {
             v[assign_v_r_w_o_i,assign_v_r_w_o_j_1]:=* (val+assign_v_r_w_o_index);
     ...
     assign_v_r_w_o_i<==0 and skip;
     
     while( (assign_v_r_w_o_i<5) )
     {
         int assign_v_r_w_o_j_2<==0 and skip;
         
         while( (assign_v_r_w_o_j_2<1) )
         {
             w[assign_v_r_w_o_i,assign_v_r_w_o_j_2]:=* (val+assign_v_r_w_o_index);
             assign_v_r_w_o_index:=assign_v_r_w_o_index+1;
             assign_v_r_w_o_j_2:=assign_v_r_w_o_j_2+1
     ...
     while( (assign_v_r_w_o_i<5) )
     {
         r[assign_v_r_w_o_i]:=* (val+assign_v_r_w_o_index);
         assign_v_r_w_o_index:=assign_v_r_w_o_index+1;
         assign_v_r_w_o_i:=assign_v_r_w_o_i+1
     ...
     while( (assign_v_r_w_o_i<1) )
     {
         o[assign_v_r_w_o_i]:=* (val+assign_v_r_w_o_index);
         assign_v_r_w_o_index:=assign_v_r_w_o_index+1;
         assign_v_r_w_o_i:=assign_v_r_w_o_i+1
 ...
 {
     frame(main_percent_old,main_percents,main_index,main_1_temp$_1,main_i,return) and ( 
     int return<==0 and skip;
     in_file_1:=fopen("in.txt","r");
     in_file_2:=fopen("out.txt","r");
     in_file_3:=fopen("modified_param_000100.txt","r");
     float *main_percents and skip;
     main_percents:=(float *)malloc(3*sizeof(float));
     ...
     while( (main_1_temp$_1) )
     {
		 ...
		 output("main_percents[", main_index, "]:", main_percents[main_index], "\n") and skip;
     ...
     while( (main_i<3) )
     {
         if(main_percents[main_i]<main_percent_rand) then 
         {
     ...
     output ("\n") and skip;
     output ("percent_old: ",main_percent_old,"\n") and skip;
     output ("percent_rand: ",main_percent_rand,"\n") and skip;
