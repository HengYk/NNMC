frame(v,w,r,o,g,e,f,x,b,y,yc,bpnn_sim_single_iter,Ek,bpnn_sim_single_Ek_,bpnn_sim_single_Ek_out,aveEk) and (
  ...
  function bpnn_sim_single (  )
 {
     ...
     while( break$=0 AND (bpnn_sim_single_Ek_out>aveEk) )
     {
         ...
         while( (bpnn_sim_single_1_i_2<3) )
         {
             ...
             {
                 bpnn_sim_single_ax[bpnn_sim_single_1_i_2]:=bpnn_sim_single_ax[bpnn_sim_single_1_i_2]+0.00000100
             }
             else 
             {
                  skip 
		 ...
		 if (bpnn_sim_single_iter >= 1000) then{
			break$<==1 and skip
		 }
		 else {
			skip	
		 ...
		 // Test...
		 if (bpnn_sim_single_iter > 1000 OR bpnn_sim_single_Ek_ <= aveEk OR bpnn_sim_single_Ek_out <= aveEk) then {
